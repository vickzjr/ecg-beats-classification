...
import numpy as np
title = 'Deep AE 32 IncartDB 3 Class'
model = '{0} dengan data IncartDB non bwr batch 8 - DNN 3 HL 3 class AAMI v1'.format(title)
beats = np.loadtxt('F:/Aritmia/new-aritmia/Dataset/Incart DB/Beats encoder {0}.csv'.format(title),delimiter=',')
labels = np.loadtxt('F:/Aritmia/new-aritmia/Dataset/Incart DB/Labels IncartDB non bwr 3 class.csv')
...
train_data,test_data,train_label, test_label = train_test_split(beats,labels,test_size=0.1, stratify=labels)
...
dnn.fit(train_data,train_label,epochs=100,batch_size=32,validation_data=(test_data,test_label),shuffle=True,callbacks=[mc])
